		...
		case NetChange::CHEAT_WARP:
			{
				...
				{
					ERROR(Format("Update server: CHEAT_WARP from %s, invalid inside building index %u.", info.name.c_str(), building_index));
					StreamError();
				}
				else
				{
...
void Game::Server_Say(BitStream& stream, PlayerInfo& info, Packet* packet)
{
	...
	{
		// id gracza jest ignorowane przez serwer ale moøna je sprawdziÊ
		assert(id == info.id);
...
void Game::Net_OnNewGameServer()
{
	...
	minimap_reveal_mp.clear();
	if(!mp_load)
		net_changes.clear(); // przy wczytywaniu jest czyszczone przed wczytaniem i w net_changes sπ zapisane quest_items
	if(!net_talk.empty())
		StringPool.Free(net_talk);
	net_changes_player.clear();
